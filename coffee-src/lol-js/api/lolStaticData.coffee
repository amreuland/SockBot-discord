...
exports.methods = {
    ...
    getChampions: (region, options={}) ->
        ...
        cacheParams = {
            key: "#{api.fullname}-champions-#{region}-" +
                "#{if options.dataById then 't' else 'f'}#{if options.champData then '-' else ''}#{(options.champData ? []).join ','}"
            api, region,
            ttl: 3600
            objectType: 'champions', params: requestParams.queryParams
        }
    ...
    #
    getChampionById: (region, id, options={}) ->
